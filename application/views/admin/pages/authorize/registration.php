              ...
              <div class="box-body">
                    ...
                    <select class="form-control" name="accType" id="accType">
                        <?php
                            foreach ($custLevel as $key => $value) {
                                $selected = ($value['ID'] == 2) ? "selected" : NULL;
                ...
                <div id="form-section-1" class="form-section" style="display:none">
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-8">
                      <input type="password" class="form-control" placeholder="" id="custPassword">
                    </div>
                  </div>
                  <div class="form-group">
                    <label class="col-sm-2 control-label">Password Confirmation</label>
                    <div class="col-sm-8">
                      <input type="password" class="form-control" placeholder="" id="custPasswordConfitmation">
                    </div>
                  </div>
                  <div class="form-group">
                    <label class="col-sm-2 control-label">PIN</label>
                    <div class="col-sm-8">
                      <input type="password" class="form-control" placeholder="" id="custPIN" maxlength="6">
                    </div>
                  </div>
                </div>
                ...
                <div id="form-section-2" class="form-section" style="display:none">
                  <div class="form-group">
                    ...
                    <div class="col-sm-5">
                        <select class="form-control" name="custProv" id="custProv" onchange="getCity('custProv', 'custCity')">
                ...
                <div id="form-section-3" class="form-section" style="display:none">
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-5">
                        <select class="form-control" name="custProvUsaha" id="custProvUsaha" onchange="getCity('custProvUsaha','custCityUsaha')">
                          <option value="0">- Pilih Provinsi -</option>
                          <?php
                            foreach ($provinsiDataList as $keyP => $valueP) {
                              echo "<option value='".$valueP['ID_PROVINSI']."'>".$valueP['NAMA_PROVINSI']."</option>";
...
<script type="text/javascript">
  ...
  function setFormSection(){
	...
	function getCity(element, cityElementID){
		var provId = $('#' + element).val();
		if(provId != 0){
			$.ajax({
		        type: "POST",
  ...
  function submitFormRegistrasi(){
    ...
    var allInputs = {
      ...
      custPasswordConfitmation: $('#custPasswordConfitmation').val(),
      custPIN: $('#custPIN').val(),
    ...
    for(var key in allInputs){
      if(selectedRules.indexOf(key) >= 0){
        ...
        if(allInputs[key] == '' || allInputs[key] == 0 || allInputs[key] == 'undefined'){
          ...
          validationMsg = 'Please Fill all field.';
          break;
        }
        // pass match
        else if(allInputs['custPassword'] != allInputs['custPasswordConfitmation']){
          validationStatus = false;
          validationMsg = 'Password does not match.';
          break;
        }
        // pass length
        else if(! (allInputs['custPassword'].length >= 6)){
          validationStatus = false;
          validationMsg = 'Password must longer than 6 character.';
          break;
        }
        // pass length
        else if(! (allInputs['custPIN'].length >= 6)){
          validationStatus = false;
          validationMsg = 'PIN must no longer than 6 character.';
          break;
        }
      }
    }
