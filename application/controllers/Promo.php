...
class Promo extends MY_Controller {
	...
	protected $img_url = 'https://admin.tmoney.co.id/assets/images/';
	...
	public function tes(){
		var_dump(is_dir($this->merchant_img_dir));
	}
	...
	public function merchant_detail($id){
		$this->db->where('f_merchant_to_category.MCL_ID',$id);
		$this->db->select('f_merchant_list.*, GROUP_CONCAT(f_merchant_category.MCT_NAME) as MCT_NAME');
		$this->db->join('f_merchant_to_category','f_merchant_list.MCL_ID = f_merchant_to_category.MCL_ID','left');
		$this->db->join('f_merchant_category','f_merchant_to_category.MCT_ID = f_merchant_category.MCT_ID','left');
		$this->db->group_by('f_merchant_list.MCL_ID');
	...
	public function merchant_add(){
		...
		if ($this->input->post()) {
			...
			if ($this->form_validation->run()) {
		        ...
		        //save category
		        foreach ($this->input->post('MCT_ID') as $key => $value) {
		        	$this->db->insert('f_merchant_to_category',array('MCT_ID'=>$value,'MCL_ID'=>$merchant_id));
		        }
	...
	public function merchant_edit($id){
		...
		$this->data['category']  = $this->db->get('f_merchant_category')->result_array();
		//get merchant
		$this->db->select('f_merchant_list.*, GROUP_CONCAT(f_merchant_to_category.MCT_ID) as category_id');
		$this->db->join('f_merchant_to_category','f_merchant_list.MCL_ID = f_merchant_to_category.MCL_ID','LEFT');
		$this->data['merchant']  = $this->db->where('f_merchant_list.MCL_ID',$id)->get('f_merchant_list')->row_array();
		$this->data['merchant']['category']  = explode(',', $this->data['merchant']['category_id']);
		//check
		if (empty($this->data['merchant']))show_404();
		if ($this->input->post()) {
			$this->load->library('form_validation');
			...
			if ($this->form_validation->run()) {
		        ...
		        //save merchant
		         $this->db->where('MCL_ID',$id)->delete('f_merchant_to_category');
		        foreach ($this->input->post('MCT_ID') as $key => $value) {
		        	$this->db->insert('f_merchant_to_category',array('MCT_ID'=>$value,'MCL_ID'=>$id));
		        }
			...
			if ($this->form_validation->run()) {
	            ...
	            if($_FILES['icon']['error']==0) {
	                ...
	                if ( ! $this->upload->do_upload('icon')){
	                    ...
	                    $data =  $this->upload->data();
	                    $default_params['MCT_ICON'] = $data['file_name'];
	                }
	            }
		        ...
		        //save merchant
		        foreach ($this->input->post('MCL_ID') as $key => $value) {
		        	$this->db->insert('f_merchant_to_category',array('MCL_ID'=>$value,'MCT_ID'=>$category_id));
		        }
	...
	public function category_edit($id){
		$this->db->select('f_merchant_category.*, GROUP_CONCAT(f_merchant_to_category.MCL_ID) as merchant_id');
		$this->db->join('f_merchant_to_category','f_merchant_category.MCT_ID = f_merchant_to_category.MCT_ID','LEFT');
		$this->data['category']  = $this->db->where('f_merchant_category.MCT_ID',$id)->get('f_merchant_category')->row_array();
		$this->data['category']['merchant']  = explode(',', $this->data['category']['merchant_id']);
		$this->data['merchant'] = $this->db->get('f_merchant_list')->result_array();
		if ($this->input->post()) {
			$this->load->library('form_validation');
			$this->form_validation->set_rules('MCT_NAME','Category Name','required');
			if ($this->form_validation->run()) {
	            ...
	            if($_FILES['icon']['error']==0) {
	                ...
	                if ( ! $this->upload->do_upload('icon')){
	                    ...
	                    $data =  $this->upload->data();
	                    $edit_data['MCT_ICON'] = $data['file_name'];
	                }
	            }
		        ...
		        //save merchant
		        $this->db->where('MCT_ID',$id)->delete('f_merchant_to_category');
		        foreach ($this->input->post('MCL_ID') as $key => $value) {
		        	$this->db->insert('f_merchant_to_category',array('MCL_ID'=>$value,'MCT_ID'=>$id));
		        }
