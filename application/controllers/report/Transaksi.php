...
class Transaksi extends MY_Controller {
    ...
    public function view_report(){
       ...
       'child_menu_active'  => 'report_transaksi',
       'other_menu_active' => 'view_report_transaksi');
        $this->session->set_flashdata($sessionMenu);
        $this->data['content_page'] = 'admin/pages/report/report_transaksi_form';
    ...
    function get_report(){
        ...
        $dateTo = date("Y-m-d 00:00:00", strtotime($dateRange[1]));
        $reportData['data'] = $this->admin_model->getReportRange($trxCode, $dateFrom, $dateTo);
    ...
    public function export_to_pdf(){
        if( $export_query = base64_decode($this->input->get('data',TRUE),TRUE) ){
            ...
            $pdf_template = $this->load->view('admin/pages/report/report_transaksi_pdf_template', $data, TRUE);
    ...
    public function search_report(){
       ...
       'parent_menu_active'  => 'report',
       'child_menu_active'  => 'report_transaksi',
       'other_menu_active' => 'search_report_transaksi');
        $this->session->set_flashdata($sessionMenu);
        $this->data['content_page'] = 'admin/pages/report/report_transaksi_search';
        $this->data['title_page'] = 'Search Report';
        $this->data['menuListData'] = $this->menuSidebar();
        $this->load->view('admin/layout', $this->data);

    }
    function get_search_report(){
        $this->checkPostData();
